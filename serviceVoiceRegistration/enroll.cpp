...
Enroll::Enroll() {
    std::cout << "initial Enroll object" << std::endl;
}

Enroll::~Enroll() {
    std::cout << "destructor Enroll object" << std::endl;
}

void Enroll::enrollVoice(std::string speakerid, std::string voiceDir, std::string fileName, int requestType, VoiceRegisterResponseMsg* responseMsg) {
    ...
    std::string spkid = speakerid; // "30"; // speaker id
    std::string train_speech = voiceDir + "/" + fileName; //"./examples/example_data/enroll/30/30-Mark_Zuckerberg_20s_2.wav"; // speech waveform
    std::string ivPath = voiceDir + "/iv";
    std::cout << "train file: " << train_speech << std::endl;
    std::ifstream is(train_speech, std::ios_base::binary); // test input, 16s speech
    WaveData wave;
    wave.Read(is); // read speech waveform
    std::cout << "boss test: waveDate has numRows: " << wave.Data().NumRows() << std::endl;
//    KALDI_ASSERT(wave.Data().NumRows() == 1);
    if(wave.Data().NumRows() != 1) {
        std::cout << "waveData has numRows != 1 => cannot process" << std::endl;
        responseMsg->set_errorcode(91);
        responseMsg->set_additionalinfo("file faild format!");
        return;
    }
    ...
    std::string old_iv = ivPath + "/train_iv.ark";
    std::string old_num_utts = ivPath + "/train_num_utts.ark";
    std::string new_iv_scp = ivPath + "/train_iv_new.scp";
    std::string new_iv = ivPath + "/train_iv_new.ark";
    std::string new_num_utts = ivPath + "/train_num_utts_new.ark";

    createFolder(ivPath);

    std::ifstream ifile(old_iv);
    if (!ifile) { // create model file if none exists
        std::string ivector_specifier = "ark:" + ivPath + "/train_iv.ark";
        std::string num_utts_specifier = "ark:" + ivPath + "/train_num_utts.ark";
    ...
    int32 vad_seg = 0;
    // vad every 10ms
    std::cout <<"boss test - wave_length: " << wave_length << ", audio_segment: " << audio_segment << ", num_segment: " << num_segment << std::endl;
    ...
    TrainProcess(spkid, &features, ivPath, requestType, responseMsg); // (if no VAD) Training process
    ...
    responseMsg->set_additionalinfo("success register");
}

bool Enroll::process_vad(Vector<BaseFloat> wave_piece_buffer, Fvad *vad, size_t framelen)
{
...
void Enroll::MfccInitiation(WaveData *wave, MfccOptions *op) {
...
void Enroll::MfccProcess(Matrix<BaseFloat> *feats, Matrix<BaseFloat> *processed_feats) {
...
void Enroll::SelectPost(Matrix<BaseFloat> *feats, Posterior *post) {
...
void Enroll::ExtractIvectors(Matrix<BaseFloat> *feats, Posterior *post, Vector<BaseFloat> *ivector) {
    using namespace kaldi;
    using std::vector;
    typedef kaldi::int32 int32;
    typedef kaldi::int64 int64;
...
void Enroll::UpdateSpkModel(std::string spkid, Vector<BaseFloat> *ivector, std::string ivPath, int requestType, VoiceRegisterResponseMsg* responseMsg) {
    using namespace kaldi;
    using std::vector;
    typedef kaldi::int32 int32;
    typedef kaldi::int64 int64;
//    std::string old_ivector_specifier = "ark:examples/iv/train_iv.ark",
//            old_num_utts_specifier = "ark:examples/iv/train_num_utts.ark",
//            new_ivector_specifier = "ark,scp:examples/iv/train_iv_new.ark,examples/iv/train_iv_new.scp",
//            new_num_utts_specifier = "ark,t:examples/iv/train_num_utts_new.ark";

    std::string old_ivector_specifier = "ark:" + ivPath + "/train_iv.ark";
    std::string old_num_utts_specifier = "ark:" + ivPath + "/train_num_utts.ark";
    std::string new_ivector_specifier = "ark,scp:" + ivPath + "/train_iv_new.ark" + ","+ ivPath + "/train_iv_new.scp";
    std::string new_num_utts_specifier = "ark,t:" + ivPath + "/train_num_utts_new.ark";
...
void Enroll::TrainProcess(std::string spkid, Matrix<BaseFloat> *feats, std::string ivPath, int requestType, VoiceRegisterResponseMsg* responseMsg) {
    ...
    UpdateSpkModel(spkid, &ivector, ivPath, requestType, responseMsg); // Update/Create speaker model for speaker id
}

bool Enroll::removeFolder(std::string path)
{
    try {
...
bool Enroll::createFolder(std::string path)
{
    try {
